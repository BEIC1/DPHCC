...
data = pd.read_excel(r'./data/radiomics-icc0.9.xlsx')
...
while flag < 2:
    ...
    function_set = ['add', 'sub', 'mul', 'div',
                    ...
                    'max', 'min']
    gp = SymbolicTransformer(generations=5, population_size=2000,
                             hall_of_fame=200, n_components=n_components,
                             function_set=function_set,
                             metric='spearman',
                             parsimony_coefficient='auto',
                             max_samples=0.8, verbose=1,
                             random_state=rs, n_jobs=3)
    gp_features = gp.fit_transform(X, y)
    new_feature_name = [str(i)+'V' for i in range(1, n_components+1)]
    new_feature = pd.DataFrame(gp_features, columns=new_feature_name)
    X = pd.concat([X, new_feature], axis=1)
    features = X.columns
    print("Number of features after symbolic regression:%s" % len(features))
...
# plt.savefig("radiomics_model.png", dpi=500)
plt.show()